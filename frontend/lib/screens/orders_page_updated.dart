    ...
    _newOrderSubscription = _realtimeService.newOrderStream.listen((newOrder) {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          final existingIndex =
              _orders.indexWhere((order) => order.id == newOrder.id);
          if (existingIndex == -1) {
        ...
        Future.delayed(const Duration(milliseconds: 500), () async {
          ...
          if (mounted &&
            ...
            debugPrint(
                '🔄 Refreshing orders from server after new order notification');
      ...
      if (mounted) {
        setState(() {
          if (preserveNewOrders) {
            ...
            debugPrint(
                '📋 Merged ${localNewOrders.length} local new orders with ${orders.length} server orders');
          } else {
            _orders = orders;
          }
          _isLoading = false;
