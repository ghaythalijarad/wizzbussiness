...
Future<void> main() async {
  ...
  final url = 'https://72nmgq5rc4.execute-api.us-east-1.amazonaws.com/dev/categories/business-type/store';
    ...
    if (response.statusCode == 200) {
      ...
      if (data['categories'] != null) {
        ...
        for (int i = 0; i < categories.length; i++) {
          ...
          print('   ${i + 1}. ${category['name']} (ID: ${category['categoryId']})');
        }
        ...
        for (final categoryJson in categories) {
          try {
            ...
            final parsedCategory = {
              'id': categoryJson['categoryId'] ?? categoryJson['category_id'] ?? categoryJson['id'] ?? '',
              'name': categoryJson['name'] ?? '',
              'businessType': categoryJson['businessType'] ?? categoryJson['business_type'] ?? '',
              'description': categoryJson['description'],
              'sortOrder': categoryJson['sort_order'] ?? 0,
            };
            
            print('   ✅ Parsed: ${parsedCategory['name']} (ID: ${parsedCategory['id']})');
          } catch (e) {
